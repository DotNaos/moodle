...
export function finalTaskOutputNodeData({
  ...
  return {
    ...
    inputs: [{ cardinality: "array", label: "task draft[]", detail: group.title, valueType: "TaskDraft" }],
    bodyData: taskOutputBodyData({ group, lossDiagnostics, outputs }),
    outputs: outputs.map((output) => ({
      cardinality: "single" as const,
      label: output.title,
      detail: output.taskId,
      state: outputState(output.status, validationProblems.length),
      valueType: "WebsiteTask",
    })),
...
export function finalScriptOutputNodeData({
  ...
  return {
    ...
    inputs: [{ cardinality: "array", label: "script section[]", detail: resource.name, valueType: "ScriptSection" }],
    ...
    outputs: outputs.map((output) => ({
      ...
      detail: output.id,
      state: outputState(output.status, validationProblems.length),
      valueType: "WebsiteScriptSection",
    })),
