...
const (
	...
	codexResourceCacheMount  = "/mnt/moodle-resource-cache"
)

var curatedWriteLocks sync.Map
...
func RunStage(courseID string, resources []moodle.Resource, stage string, options RunOptions) (contract.StudyPipelineResponse, error) {
	...
	case "curated":
		...
		curation, err = runCodexCuration(root, courseID, resources, plan, options, now)
		if err != nil {
			stageErr = err
		}
		accountability, err = buildCuratedAccountability(root, courseID, resources, plan.Materials, now, curation)
		if err != nil {
			return contract.StudyPipelineResponse{}, err
		}
	default:
	...
	response.ConfigHash = runConfigHash(stage, response.Engine, options.ConfigHash, options.Model, options.ReasoningEffort)
	if stage == "curated" {
		...
		if stageErr != nil {
			markStudyPipelineResponseFailed(&response, stageErr)
			return response, stageErr
		}
		if err := accountability.CompletionError(); err != nil {
			markStudyPipelineResponseFailed(&response, err)
			return response, err
		}
		if curation != nil {
			if err := materializeCodexCuration(root, courseID, curation, response.Materials, now); err != nil {
				markStudyPipelineResponseFailed(&response, err)
				return response, err
			}
			if err := writeCurated(root, courseID, resources, now); err != nil {
...
func markStudyPipelineResponseFailed(response *contract.StudyPipelineResponse, err error) {
	if response == nil {
		return
	}
...
func writeCurated(root string, courseID string, resources []moodle.Resource, now time.Time) error {
	...
	lockKey := filepath.Join(root, courseID)
	lockValue, _ := curatedWriteLocks.LoadOrStore(lockKey, &sync.Mutex{})
...
func buildScript(root string, courseID string, plan contract.StudyPipelineResponse, now time.Time) string {
	...
	for _, material := range plan.Materials {
		...
		if content == "" {
			...
			out.WriteString(placeCuratedVisualAssets(root, courseID, material, content))
			out.WriteString("\n\n")
		}
	}
	if out.Len() == 0 {
...
func taskPrompt(root string, courseID string, link contract.StudyPipelineTaskLink) string {
	...
	if content != "" {
		lines = append(lines, placeCuratedVisualAssets(root, courseID, link.Task, content))
	} else {
...
func solutionPrompt(root string, courseID string, resource contract.StudyPipelineMaterial) string {
	...
	content = placeCuratedVisualAssets(root, courseID, resource, content)
	lines := []string{
		"---",
...
func curatedVisualAssetMarkdown(root string, courseID string, material contract.StudyPipelineMaterial, existingOutput string) string {
	figures := missingCuratedVisualAssetFigures(root, courseID, material, existingOutput)
	if len(figures) == 0 {
		return ""
	}
	return "## PDF-Bilder\n\n" + strings.Join(curatedFigureMarkdown(figures), "\n\n")
}

type curatedVisualFigure struct {
	asset    contract.DocumentAsset
	markdown string
}

func missingCuratedVisualAssetFigures(root string, courseID string, material contract.StudyPipelineMaterial, existingOutput string) []curatedVisualFigure {
	...
	for _, asset := range document.Assets {
		...
		figures = append(figures, curatedVisualFigure{asset: asset, markdown: curatedAssetFigure(courseID, asset)})
	}
	return figures
}

func placeCuratedVisualAssets(root string, courseID string, material contract.StudyPipelineMaterial, existingOutput string) string {
	base := stripGeneratedPDFImagesSection(strings.TrimSpace(existingOutput))
	figures := missingCuratedVisualAssetFigures(root, courseID, material, base)
	if len(figures) == 0 {
		return base
	}
	updated, placed := insertCuratedFiguresNearVisualReference(base, figures)
	if len(placed) == len(figures) {
		return updated
	}
	remaining := figures[len(placed):]
	return strings.TrimRight(updated, "\n") + "\n\n## PDF-Bilder\n\n" + strings.Join(curatedFigureMarkdown(remaining), "\n\n")
}

func curatedFigureMarkdown(figures []curatedVisualFigure) []string {
	out := make([]string, 0, len(figures))
	for _, figure := range figures {
		out = append(out, figure.markdown)
	}
	return out
}

func insertCuratedFiguresNearVisualReference(markdown string, figures []curatedVisualFigure) (string, []curatedVisualFigure) {
	if strings.TrimSpace(markdown) == "" || len(figures) == 0 {
		return markdown, nil
	}
	index := visualReferenceIndex(markdown)
	if index < 0 {
		return markdown, nil
	}
	insertAt := paragraphEnd(markdown, index)
	figureBlock := strings.Join(curatedFigureMarkdown(figures), "\n\n")
	updated := strings.TrimRight(markdown[:insertAt], "\n") + "\n\n" + figureBlock + "\n\n" + strings.TrimLeft(markdown[insertAt:], "\n")
	return updated, figures
}

func visualReferenceIndex(markdown string) int {
	lower := strings.ToLower(markdown)
	for _, keyword := range []string{"diagramm", "diagram", "abbildung", "figure", "chart"} {
		if index := strings.Index(lower, keyword); index >= 0 {
			return index
		}
	}
	for _, keyword := range []string{"bestimmen sie", "berechnen sie", "entscheiden sie"} {
		if index := strings.Index(lower, keyword); index >= 0 {
			return index
		}
	}
...
func paragraphEnd(markdown string, from int) int {
	if from < 0 || from >= len(markdown) {
		return len(markdown)
	}
	if offset := strings.Index(markdown[from:], "\n\n"); offset >= 0 {
		return from + offset
	}
	return len(markdown)
}

func stripGeneratedPDFImagesSection(markdown string) string {
	marker := "\n## PDF-Bilder"
	index := strings.Index(markdown, marker)
	if index < 0 {
		if strings.HasPrefix(markdown, "## PDF-Bilder") {
			return ""
		}
		return strings.TrimSpace(markdown)
	}
	return strings.TrimSpace(markdown[:index])
}
...
func appendMissingCuratedVisualAssets(root string, courseID string, material contract.StudyPipelineMaterial, outputPath string) error {
	...
	content := string(data)
	updated := placeCuratedVisualAssets(root, courseID, material, content)
	if updated == strings.TrimSpace(content) {
		return nil
	}
	return os.WriteFile(outputPath, []byte(strings.TrimRight(updated, "\n")+"\n"), 0o644)
}
...
func isCodexAuthError(values ...string) bool {
	for _, value := range values {
		text := strings.ToLower(value)
		if strings.Contains(text, "not logged in") ||
			strings.Contains(text, "401 unauthorized") ||
			strings.Contains(text, "missing bearer or basic authentication") ||
			strings.Contains(text, "authentication required") ||
			strings.Contains(text, "please run `codex login`") {
			return true
		}
	}
	return false
...
func runDockerCodexWithOptions(ctx context.Context, options dockerCodexOptions) (string, error) {
	...
	args = append(args, options.Image, "sh", "-lc", options.Command)
...
func appendCodexResourceCacheMount(args []string) []string {
	cacheRoot := ResourceCacheRootFromEnv()
	if cacheRoot == "" {
		return args
	}
	...
	return append(args, "-e", "MOODLE_RESOURCE_CACHE_MOUNT="+codexResourceCacheMount, "-v", dockerHostMountPath(cacheRoot)+":"+codexResourceCacheMount+":ro")
}

func ResourceCacheRootFromEnv() string {
	if value := strings.TrimSpace(os.Getenv(EnvResourceCacheRoot)); value != "" {
		return value
	}
	return filepath.Join(ArtifactRootFromEnv(), "material-cache")
}
...
func prepareCodexStateRoot(artifactRoot string, userID string) (string, error) {
	...
	stateRoot := filepath.Join(codexStateBaseRoot(artifactRoot), "codex-users", userSegment)
...
func emitCodexLineEvent(line string, emit func(contract.StudyPipelineRefineEvent)) {
	...
	if err := json.Unmarshal([]byte(cleanLine), &event); err == nil {
		...
		if category, text := classifyCodexTextEvent(event); category != "status" {
			refineEvent.Category = category
			refineEvent.ContentPreview = text
		} else if category, title, status, id := classifyCodexToolEvent(event); category == "tool" {
...
func classifyCodexTextEvent(event map[string]any) (category, text string) {
	switch eventType := stringValue(event["type"]); eventType {
	case "response.output_text.delta", "message.delta", "agent_message.delta":
		text = firstNonEmpty(
	...
	case "response.output_text.done", "message.completed", "agent_message.completed":
		text = firstNonEmpty(
			stringValue(event["text"]),
			stringValue(event["content"]),
	...
	case "item.updated", "item.completed":
		...
		if !ok || stringValue(item["type"]) != "agent_message" {
			return "status", ""
		}
		text = firstNonEmpty(
			stringValue(item["text"]),
			stringValue(item["message"]),
			codexContentText(item["content"]),
		)
		if text != "" {
			return "message", text
		}
...
func codexContentText(value any) string {
	...
	case []any:
		...
		for _, part := range content {
			if text := codexContentText(part); text != "" {
	...
	case map[string]any:
		return firstNonEmptyRaw(
			codexRawStringValue(content["text"]),
			codexRawStringValue(content["content"]),
			codexRawStringValue(content["delta"]),
		)
	default:
		return ""
	}
...
func codexRawStringValue(value any) string {
	text, ok := value.(string)
	if !ok {
		return ""
	}
...
func runConfigHash(stage string, engine string, configHash string, model string, reasoningEffort string) string {
	...
	if strings.TrimSpace(stage) == "curated" {
		model = strings.TrimSpace(model)
		if model != "" {
			effort := strings.TrimSpace(reasoningEffort)
			if effort != "" {
				return "config:" + strings.TrimSpace(stage) + ":" + engine + ":" + safeSegment(model) + ":" + safeSegment(effort)
			}
			return "config:" + strings.TrimSpace(stage) + ":" + engine + ":" + safeSegment(model)
		}
	}
