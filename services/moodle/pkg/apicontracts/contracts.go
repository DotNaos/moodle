...
func NormalizeStudyPipelineStage(stage string) string {
	trimmed := strings.TrimSpace(stage)
	switch strings.ToLower(trimmed) {
	case "resources", "resource-set":
		return "inventory"
	case "raw-import", "raw_import":
		return "raw"
	case "extraction", "extract", "ocr":
		return "extracted"
	case "codex", "curation", "codex-transform", "curated-ready":
		return "curated"
	default:
		return trimmed
	}
...
type StudyPipelineRefineEvent struct {
	...
	// command, MCP tool, web search), an assistant text "message"/"delta", or
	// "status" lifecycle noise. Empty means status. ToolTitle/ToolStatus/ToolID
	// carry the surfaced tool-call details; ContentPreview carries streamed text.
